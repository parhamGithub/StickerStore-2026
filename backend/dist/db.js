...
import fs from "fs";
const DB_PATH = path.join(path.dirname(new URL(import.meta.url).pathname), "..", "data", "store.db");
let db;
...
function seedIfEmpty() {
    const row = db.query("SELECT COUNT(*) as count FROM products").get();
    if (row.count > 0)
        return;
  ...
  `);
    ...
    const seedTx = db.transaction(() => {
        for (const p of products) {
            insertProduct.run(p.id, p.name, p.category, p.price, p.material, p.size, p.bgColor, p.rotation, p.image, p.badge ?? null);
        }
    ...
    const existingUser = db
        ...
        .get("test@stickerly.com");
    if (!existingUser) {
        const id = crypto.randomUUID();
        const passwordHash = Bun.password.hashSync("password123");
        db.run("INSERT INTO users (id, name, email, password) VALUES (?, ?, ?, ?)", [id, "Test User", "test@stickerly.com", passwordHash]);
